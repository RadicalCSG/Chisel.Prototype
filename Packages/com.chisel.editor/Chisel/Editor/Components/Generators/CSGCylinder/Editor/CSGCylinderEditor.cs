					...
					switch (generator.Type)
					{
						case CylinderShapeType.Cylinder:
						{
							...
							} else
							{
								...
								//bottomXVector = UnitySceneExtensions.Handles.Radius2DHandle(topPoint, normal, bottomXVector, renderDisc: false);
								//bottomXVector = UnitySceneExtensions.Handles.Radius2DHandle(bottomPoint, -normal, bottomXVector, renderDisc: false);
				...
				if (EditorGUI.EndChangeCheck())
				{
					...
					if (!generator.IsEllipsoid)
					{
						if (prevBottomZVector != bottomZVector)
							bottomXVector = Vector3.Cross(normal, bottomZVector.normalized) * bottomZVector.magnitude;
						if (prevTopZVector != topZVector)
							topXVector = Vector3.Cross(normal, topZVector.normalized) * topZVector.magnitude;
					}
					...
					if (generator.IsEllipsoid)
					{
						generator.BottomDiameterX = bottomXVector.magnitude;
						generator.BottomDiameterZ = bottomZVector.magnitude;

						generator.TopDiameterX = topXVector.magnitude;
						generator.TopDiameterZ = topZVector.magnitude;
					} else
					{
						if (prevBottomZVector != bottomZVector)
						{
							generator.BottomDiameterX = bottomZVector.magnitude;
							generator.BottomDiameterZ = bottomZVector.magnitude;
						} else
						{
							generator.BottomDiameterX = bottomXVector.magnitude;
							generator.BottomDiameterZ = bottomXVector.magnitude;
						}

						if (prevTopZVector != topZVector)
						{
							generator.TopDiameterX = topZVector.magnitude;
							generator.TopDiameterZ = topZVector.magnitude;
						} else
						{
							generator.TopDiameterX = topXVector.magnitude;
							generator.TopDiameterZ = topXVector.magnitude;
						}
					}
				}
