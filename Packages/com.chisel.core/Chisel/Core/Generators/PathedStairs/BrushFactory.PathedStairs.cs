...
namespace Chisel.Core
{
    ...
    public sealed partial class BrushMeshFactory
    {
        ...
        public static bool GeneratePathedStairs(ref ChiselBrushContainer brushContainer, ref ChiselSurfaceDefinition surfaceDefinition, ref ChiselPathedStairsDefinition definition)
        {
            ...
            GetPathVertices(definition.shape, definition.curveSegments, shapeVertices);
            ...
            for (int vi0 = (count + count - 3) % count, vi1 = (count + count - 2) % count, vi2 = (count + count - 1) % count, vi3 = 0; vi3 < count; vi0 = vi1, vi1 = vi2, vi2 = vi3, vi3++)
            {
                ...
                var depthVector0 = new float2(d0.y, -d0.x) * depth;
                var depthVector1 = new float2(d1.y, -d1.x) * depth;
                var depthVector2 = new float2(d2.y, -d2.x) * depth;
                ...
                var minMaxAABB = new MinMaxAABB { Min = definition.stairs.bounds.min, Max = definition.stairs.bounds.max };
                var description = new LineairStairsData(minMaxAABB,
                                                        definition.stairs.stepHeight, definition.stairs.stepDepth,
                                                        definition.stairs.treadHeight,
                                                        definition.stairs.nosingDepth, definition.stairs.nosingWidth,
                                                        definition.stairs.plateauHeight,
                                                        definition.stairs.riserType, definition.stairs.riserDepth,
                                                        leftSide, rightSide,
                                                        definition.stairs.sideWidth, definition.stairs.sideHeight, definition.stairs.sideDepth);
                var subMeshCount = description.subMeshCount;
                if (subMeshCount == 0)
                    continue;
