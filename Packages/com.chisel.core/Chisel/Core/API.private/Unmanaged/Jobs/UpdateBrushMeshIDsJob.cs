...
namespace Chisel.Core
{
    ...
    unsafe struct UpdateBrushMeshIDsJob : IJob
    {
        ...
        public static void UpdateBrushMeshIDs([NoAlias, ReadOnly] ref CompactHierarchy                         compactHierarchy,
                                              [NoAlias, ReadOnly] NativeHashMap<int, RefCountedBrushMeshBlob>  brushMeshBlobs,
                                              [NoAlias, ReadOnly] int                                          brushCount,
                                              [NoAlias, ReadOnly] NativeList<CompactNodeID>                    brushes,
                                              // Read/Write
                                              [NoAlias] NativeHashSet<int>                                     allKnownBrushMeshIndices,
                                              [NoAlias] NativeArray<ChiselLayerParameters>                     parameters,
                                              [NoAlias] NativeArray<int>                                       parameterCounts,
                                              // Write
                                              [NoAlias, WriteOnly] NativeArray<int>                            allBrushMeshIDs)
        {
            Debug.Assert(parameters.Length == SurfaceLayers.ParameterCount);
            Debug.Assert(parameterCounts.Length == SurfaceLayers.ParameterCount);
            Debug.Assert(SurfaceLayers.kLayerUsageFlags.Length == SurfaceLayers.ParameterCount);
            ...
            var updateBrushMeshIndices = new UnsafeList<int>(capacity, Allocator.Temp);
            {
                foreach (int brushMeshHash in allKnownBrushMeshIndices)
                    updateBrushMeshIndices.Add(brushMeshHash);
                ...
                for (int nodeOrder = 0; nodeOrder < brushCount; nodeOrder++)
                {
                    ...
                    } else
                    {
                        ...
                        if (updateBrushMeshIndices.Remove(brushMeshHash))
                            allKnownBrushMeshIndices.Remove(brushMeshHash);
                    } 
                }
                ...
                // Regular index operator will return a copy instead of a reference *sigh*
                var parameterPtr = (ChiselLayerParameters*)parameters.GetUnsafePtr();
                foreach (var brushMeshHash in updateBrushMeshIndices)
                {
                    ref var polygons = ref brushMeshBlobs[brushMeshHash].brushMeshBlob.Value.polygons;
                    for (int p = 0; p < polygons.Length; p++)
                    {
            ...
            //updateBrushMeshIndices.Dispose();
        }
    }
}
